...
# Prep free-flow data (same for both methods) ----------------------------------
...
for (agg_method in c("mean", "85th_percentile")) {
  ...
  ggsave(p, filename = file.path(figures_dir, paste0("isochrone_hex_changes_", agg_method, ".png")),
         height = 10,
         width = 9)

  ggsave(p, filename = file.path(figures_dir, paste0("figure_7_", agg_method, ".png")),
         height = 10,
         width = 9)
