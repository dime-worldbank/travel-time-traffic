...
# Load data --------------------------------------------------------------------
mode_sf <- readRDS(file.path(data_dir, "Travel Time Routes 2026", "typical_routes.Rds"))

mode_sf <- mode_sf %>%
  dplyr::mutate(distance_m = geometry %>% st_length() %>% as.numeric(),
                road_name = name %>% tools::toTitleCase())
...
route_df <- route_df %>%
  dplyr::mutate(
    dplyr::across(
      c(start_lat, start_lon, end_lat, end_lon),
      ~ round(.x, 6)
    )
  )
...
# Table ------------------------------------------------------------------------
sink(file.path(tables_dir, "route_summary_calib.tex"))
