...
# Load data --------------------------------------------------------------------
crash_df <- readRDS(file.path(analysis_data_dir, "google_twitter_50m.Rds"))
...
crash_df <- crash_df %>%
  ...
  dplyr::mutate(date = datetime %>% hour(),
                ...
                duration_in_traffic_min_ln = log(duration_in_traffic_min),
                speed_in_traffic_kmh_ln = log(speed_in_traffic_kmh),
                delay_factor_ttsample = case_when(
                  is.na(speed_in_traffic_kmh_ln) ~ NA,
                  TRUE ~ delay_factor
                ))
...
lm_delayfactor_df <- feols(
  delay_factor ~ i(hour_of_day_since_crash, crash_day, ref = -1) | crash_id + hour + dow,
  data = crash_df,
  cluster = ~crash_id
) %>%
  lm_to_df() %>%
...
lm_duration_df <- feols(
  delay_factor_od ~ i(hour_of_day_since_crash, crash_day, ref = -1) | crash_id + hour + dow,
  data = crash_df,
  cluster = ~crash_id
) %>%
  lm_to_df() %>%
...
ggsave(p1, filename = file.path(figures_dir, "lm_crash_50m.png"),
       height = 3,
       width = 9)
...
ggsave(p2, filename = file.path(figures_dir, "lm_crash_type_50m.png"),
       height = 3,
       width = 9)
...
ggsave(p, filename = file.path(figures_dir, "lm_crash_all_50m.png"),
       height = 6,
       width = 9)
