...
for (agg_method in c("mean", "85th_percentile")) {
  ...
  iso_cong_route_sf <- readRDS(file.path(data_dir, "Isochrone Routes",
                                         paste0("iso_congestion_routes_", agg_method, ".Rds"))) %>%
    dplyr::filter(hour %in% 18,
                  dow_weekday %in% T)
  iso_cong_poly_sf <- readRDS(file.path(data_dir, "Isochrone Routes",
                                        paste0("iso_congestion_poly_", agg_method, ".Rds"))) %>%
    dplyr::filter(hour %in% 18,
                  dow_weekday %in% T)
  ...
  ggsave(p,
         filename = file.path(figures_dir, paste0("isochrone_methods_", agg_method, ".png")),
         height = 11,
         width = 9.5)
  ...
  ggsave(p,
         filename = file.path(figures_dir, paste0("isochrone_methods_small_", agg_method, ".png")),
         height = 6.5,
         width = 14)
