...
p_tod <- bind_rows(
  ...
  gadm1_hr_df %>%
    dplyr::mutate(title = "C. Delay Factor\n(Traffic Level Data)\n[All Nairobi]",
                  var = delay_factor)
) %>%
  ggplot() +
...
p_map <- ggarrange(p_map_levels + theme(legend.position = "bottom"),
                   p_map_pc + theme(legend.position = "bottom"),
                   ncol = 2,
                   widths = c(0.68, 0.3))
