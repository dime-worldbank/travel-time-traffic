...
# Traffic levels
tl_df <- file.path(extracted_data_dir, "osm_10m", "google_traffic_levels") %>%
...
osm_df <- readRDS(file.path(data_dir, "OSM", "FinalData", "osm_nbo_line.Rds")) %>%
  st_drop_geometry()

tl_df <- tl_df %>% left_join(osm_df,by = "uid")

# Export
saveRDS(tl_df, file.path(analysis_data_dir, "google_osm_10m.Rds"))