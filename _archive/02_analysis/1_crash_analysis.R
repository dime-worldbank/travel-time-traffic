...
# Load data --------------------------------------------------------------------
df <- readRDS(file.path(analysis_data_dir, "ntsa_crashes_100m_wide_clean.Rds"))
...
# df %>%
#   filter(!is.na(gg_speed_in_traffic_kmh_mean)) %>%
#   pull(datetime) %>%
#   date() %>%
#   summary()
# 
# df %>%
#   filter(!is.na(mb_speed_in_traffic_kmh_mean)) %>%
#   pull(datetime) %>%
#   date() %>%
#   summary()
# 
# df$gg_speed_in_traffic_kmh_mean
# df$mb_speed_in_traffic_kmh_mean
...
## Define sample
df_sub$speed_sample <- df_sub$uid %in% (df_sub %>%
...
lm_hr_coef_df <- bind_rows(
  ...
  map_df(unique(df_sub$name), run_reg_hourly, df_sub[df_sub$speed_sample %in% T,]) %>%
    mutate(sample = "Sub")
)
