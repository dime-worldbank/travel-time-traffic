...
for(polygon_i in POLYGONS_ALL){
  ...
  mapbox_tl_df <- file.path(extracted_data_dir, polygon_i, "mapbox_traffic_levels") %>%
    ...
    dplyr::mutate(low      = low      %>% replace_na(0),
                  moderate = moderate %>% replace_na(0),
                  heavy    = heavy    %>% replace_na(0),
                  severe   = severe   %>% replace_na(0)) %>%
    dplyr::mutate(length_all = low + moderate + heavy + severe) %>%
    
    group_by(uid) %>%
    dplyr::mutate(length_all_max = max(length_all,na.rm = T)) %>%
    ungroup() %>%
